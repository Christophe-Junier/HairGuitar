...
namespace :db do
  namespace :json do
    ...
    def pro_prestation_creation(pro_prestation, this_pro)
      ProsPrestation.create(pro_id: this_pro.id,
                            prestation_id: Prestation.find_by(reference: pro_prestation).id)
    end

    # BookedPrestation creation method stands here
    def booked_prestation_creation(booked_prestation, this_booking)
      BookedPrestation.create(booking_id: this_booking.id,
                              prestation_id: Prestation.find_by(reference: booked_prestation).id)
    end
  end
end
